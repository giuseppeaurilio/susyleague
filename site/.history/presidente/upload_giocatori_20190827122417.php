	...
	if (($handle = fopen($filename, "r")) !== FALSE) {
		...
		while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			...
			try{
				...
				$queryinsertgiocatore="INSERT INTO `giocatori`(`id`, `ruolo`,`nome`,`id_squadra` ) SELECT $data[0],'$data[1]','$nome',id from squadre_serie_a where `squadra_breve`='$squadra_breve'";
