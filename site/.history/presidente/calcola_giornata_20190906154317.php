...
$(document).ready(function(){
	...
	for (i=1; i<13; i++){ //i scorre le squadre
 	var dataString = 'id_squadra='+ i + '&id_giornata=' + id_giornata + '&giocatori=';
		...
		var voto=test.find('td:eq(4)').text();
		var voto_md=test.find('td:eq(5)').text();
		var sostituzione=test.find('td:eq(6)').find('input:checkbox').prop("checked")? "1": "0";
		console.log(j + "_" + voto + "_" + voto_md + "_" + sostituzione);
...
while ($row=$result_giornata->fetch_assoc()) {
	...
	?>
        <tr contenteditable='true' id=row_<?php  echo $id_ospite  . "_" . ($i+1);?> style="background-color: <?php switch ($ruolo_giocatore) {
    case "P":
        echo "#66CC33";
        break;
    case "D":
