...
class Partita
{
    ...
    public function Partita($_idgiornata, $_idcasa, $_idospite, $_usamediadifesa, $_usafattorecasa)
    {
    ...
    {
        $risultatocasa = $this->calcolaRisultatoSquadra($this->idgiornata, $this->idcasa);
        $risultatoospite = calcolaRisultatoSquadra($this->idgiornata, $this->idospite);
    }
    ...
    private function calcolaRisultatoSquadra($idgiornata, $idsquadra)
    {
        ...
        $resultformazione = $conn->query($query1);
        if($boolprint) echo 'resultformazione'.print_r($resultformazione).'<br>';
       
        //recupero i dati dal DB e li trasferisco nell'array di oggetti
        while ($giocatore = $resultformazione->fetch_assoc()) {
        ...
        $mediadifesa = $this->mediaDifesa($numdifcv, $sumdifesa );
