...
CREATE PROCEDURE `getRisposteSquadreSondaggio`(IN `idSondaggio` INT, IN `idOpzione` INT) NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER select count(*) from sondaggi_risposte where id_sondaggio = idSondaggio and id_opzione = idOpzione;
