...
if ($num_giornate>0) {
	...
	while ($row=$result_giornate->fetch_assoc()) {
		...
		{ $descrizioneGiornata ="Coppa Italia - Girone Narpini. Giornata ". (($id-33)/3 > 4 ? floor(($id-33)/3) - 4 : floor(($id-33)/3) +1 );}
		else if ($id>48 && $id<= 63)
		{ $descrizioneGiornata ="Coppa Italia - Girone Gianluca. Giornata ". (($id-48)/3 > 4 ? floor(($id-48)/3) - 4 : floor(($id-48)/3) +1 );}
		else if ($id == 64 )
